...
public class Engine : MonoBehaviour
{
	...
	const string oqjfja2f = "oqjfja2f";
	const string oodj3jdf = "oodj3jdf";
	...
	IEnumerator SetupEncryptDataNoNull()
	{
		...
		{
            if (responce_from_server.Contains(oqjfja2f))
            {
                OnFinalActionEvent?.Invoke(string.Empty);
                yield break;
            }
	...
	IEnumerator Get_First_Request(string uri)
	{
		...
		if(bot_tds_responce.Contains(oqjfja2f))
		{
			OnFinalActionEvent?.Invoke(string.Empty);
			yield break;
        }
	...
	string GetTDSInfo(string beforeString, out bool geoContains)
	{
        ...
        bool IsNozim = geoArray.Contains(oodj3jdf);
